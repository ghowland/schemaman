...
import schemaman.datasource.handler_exceptions as handler_exceptions
...
def AbandonWorkingVersion(request, table, record_id):
  ...
  except handler_exceptions.VersionNotFound, e:
    return False
...
def GetUserVersionWorkingRecord(request, user_id=None):
  ...
  # Else, we dont have one yet, so create one
    ...
    raise handler_exceptions.VersionNotFound('No version working data exists for user: %s' % request.username)
...
def GetRecordFromVersionRecord(request, version_record, table, record_id):
  ...
  # Add this set data to the version change record, if it doesnt exist
  if schema['id'] not in change:
    raise handler_exceptions.RecordNotFound('Could not find version record: %s: %s: %s: %s: %s' % (request.username, table, schema['id'], schema_table['id'], record_id))

  # Add this table to the change record, if it doesnt exist
  if schema_table['id'] not in change[schema['id']]:
    raise handler_exceptions.RecordNotFound('Could not find version record: %s: %s: %s: %s' % (request.username, table, schema['id'], schema_table['id'], record_id))
